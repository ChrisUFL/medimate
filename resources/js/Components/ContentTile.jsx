...
const ContentTile = ({image, title, data, color, dataType = "text"}) => {
  ...
  if (dataType === 'calendar') {
    for (const [k, v] of Object.entries(data.calendar)) {
      listItems.push(<Link href={route('web.home')}><li key={v.id}> {`${v.date} - ${v.title}`} </li></Link>);
    }
  }

  if (dataType === 'medication') {
    for (const [k, v] of Object.entries(data.medication)) {
      listItems.push(<Link href={route('web.home')}><li key={v.name}> {`${v.time} - ${v.name}`} </li></Link>);
    }
  }
  ...
  return (
    <div className={`flex-col justify-center px-11 pb-10 text-white basis-3/12 ${color}`}>
        ...
        <span className='flex text-left text-wrap'>{dataType !== "text" ? <ul> { listItems } </ul> : placeHolder}</span>
    </div>
  )
}
